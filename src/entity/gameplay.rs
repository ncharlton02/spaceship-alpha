...
impl<'a> System<'a> for AsteroidFieldSystem {
    ...
    fn run(&mut self, data: Self::SystemData) {
        let (entities, lazy_update, mut ecs_utils, transforms, mut fields, meshes) = data;
        ...
        for field in (&mut fields).join() {
            ...
            field
                ...
                .for_each(|asteroid| ecs_utils.mark_for_removal(*asteroid));
            ...
            if field.tick > 0 {
                ...
                let entity = lazy_update
                    ...
                    .with(transform)
                    .with(Model::new(*meshes.asteroids.choose(&mut rng).unwrap()))
                    .with(RigidBody {
